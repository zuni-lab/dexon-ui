    ...
    const fetchQuote = async () => {
      try {
        ...
        if (!amount || amount === "0") {
          ...
          setPriceRate(
            Number(formatUnits(estUsdcAmount, Tokens.USDC.decimals)).toFixed(2),
          );
          setUsdcAmount("0");
          return;
        }
        ...
        const formattedUsdcAmount = Number(
          formatUnits(estUsdcAmount, Tokens.USDC.decimals),
        ).toFixed(2);
        setUsdcAmount(formattedUsdcAmount);

        // Calculate price rate (price per token)
        const rate = (
          Number(formatUnits(estUsdcAmount, Tokens.USDC.decimals)) /
          Number(amount)
        ).toFixed(2);
        setPriceRate(rate);
      } catch (_error) {
